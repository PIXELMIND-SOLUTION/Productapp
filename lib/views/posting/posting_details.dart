...
class _PostingDetailsState extends State<PostingDetails> with SingleTickerProviderStateMixin {
  ...
  String _extractFeature(List<dynamic>? features, String type) {
    if (features == null) return '';
    
    for (var feature in features) {
      String name = (feature['name']?.toString() ?? '').toLowerCase();
      if (type == 'bed' && (name.contains('bedroom') || name.contains('bed'))) {
        return feature['name']?.toString() ?? '';
      } else if (type == 'bath' && (name.contains('bathroom') || name.contains('bath'))) {
        return feature['name']?.toString() ?? '';
      } else if (type == 'area' && (name.contains('sqft') || name.contains('sq') || name.contains('area'))) {
        return feature['name']?.toString() ?? '';
      }
    }
    return '';
  ...
  Widget _buildPendingTab() {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        final product = pendingProducts[index];
        return _buildPropertyCard(product, isPending: true);
      },
    );
  }
  ...
  Widget _buildApprovedTab() {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        final product = approvedProducts[index];
        return _buildPropertyCard(product, isPending: false);
      },
    );
  }

  // Property Card with Home Screen Design
  Widget _buildPropertyCard(dynamic product, {required bool isPending}) {
    final images = product['images'] as List<dynamic>? ?? [];
    final imageUrl = images.isNotEmpty ? images[0].toString() : '';
    final features = product['features'] as List<dynamic>?;
    
    String bed = _extractFeature(features, 'bed');
    String bath = _extractFeature(features, 'bath');
    String area = _extractFeature(features, 'area');
                ...
                Row(
                  children: [
                    _StatChip(
                      ...
                      label: bed.isNotEmpty ? bed : '4 Bed',
                    ),
                    const SizedBox(width: 12),
                    _StatChip(
                      imagePath: 'assets/images/bath.png',
                      label: bath.isNotEmpty ? bath : '2 Bath',
                    ),
                    const SizedBox(width: 12),
                    _StatChip(
                      imagePath: 'assets/images/sqft.png',
                      label: area.isNotEmpty ? area : '7,500 sqft',
                    ),
                  ],
                ),
  ...
  void _showOptionsMenu(BuildContext context, dynamic product) {
    showModalBottomSheet(
      context: context,
      shape: const RoundedRectangleBorder(
        borderRadius: BorderRadius.vertical(top: Radius.circular(20)),
      ...
      builder: (context) {
        return Container(
          ...
          child: Column(
            ...
            children: [
              ListTile(
                leading: const Icon(Icons.edit, color: Color(0xFF1CA7C6)),
                title: const Text('Edit Listing'),
                onTap: () {
                  Navigator.pop(context);
                  // TODO: Navigate to edit screen
...
// Stat Chip Widget (matching home screen design)
