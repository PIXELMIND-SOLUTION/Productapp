...
class _PostingDetailsState extends State<PostingDetails>
  ...
  Future<void> _loadProducts() async {
    ...
    try {
      final response = await http.get(
        Uri.parse('${ApiConstants.baseUrl}/api/user/$userId'),
        headers: {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true) {
          ...
          setState(() {
            ...
            // Categorize products based on status field
            ...
            approvedProducts = products
                .where(
                    (p) => p['status'] == 'approved' || p['isApproved'] == true)
                .toList();

            rejectedProducts =
                products.where((p) => p['status'] == 'rejected').toList();

            isLoading = false;
          });

          print('📊 Products loaded:');
          print('   Pending: ${pendingProducts.length}');
          print('   Approved: ${approvedProducts.length}');
          print('   Rejected: ${rejectedProducts.length}');
        } else {
  ...
  Future<void> _updateProductStatus(String productId, bool isActive) async {
    ...
    try {
      final response = await http.patch(
        Uri.parse('${ApiConstants.baseUrl}/api/$productId/status'),
        headers: {
          'Authorization': 'Bearer $token',
          'Content-Type': 'application/json',
        },
      ...
      print("pppppppppppppppppppp${response.body}");
      ...
      if (response.statusCode == 200) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content:
                  Text(isActive ? 'Product activated' : 'Product deactivated'),
  ...
  Future<void> _deleteProduct(String productId) async {
    ...
    try {
      final response = await http.delete(
        Uri.parse('${ApiConstants.baseUrl}/api/$productId'),
        headers: {
          'Authorization': 'Bearer $token',
          'Content-Type': 'application/json',
        },
  ...
  String _getProductStatus(dynamic product) {
    if (product['status'] != null) {
      return product['status'].toString();
    }
    // Fallback to isApproved
    return product['isApproved'] == true ? 'approved' : 'pending';
  }
  ...
  String _getEmptyStateDescription(String status) {
    switch (status) {
      ...
      case 'rejected':
        return 'You don\'t have any rejected properties.\nIf your listing is rejected, it will appear here with the reason.';
      default:
              ...
              child: Container(
                ...
                child: TabBar(
                  ...
                  tabs: [
                      ...
                      child: Row(
                        ...
                        children: [
                          Container(
                            ...
                            height: 8,
                            decoration: BoxDecoration(
                              color: pendingProducts.isNotEmpty
                                  ? Colors.orange
                                  : Colors.transparent,
                      ...
                      child: Row(
                        ...
                        children: [
                          Container(
                            ...
                            height: 8,
                            decoration: BoxDecoration(
                              color: approvedProducts.isNotEmpty
                                  ? Colors.green
                                  : Colors.transparent,
                              shape: BoxShape.circle,
                            ),
                    ...
                    //         height: 8,
                    //         decoration: BoxDecoration(
                    //           color: rejectedProducts.isNotEmpty ? Colors.red : Colors.transparent,
                    //           shape: BoxShape.circle,
                    //         ),
                    //       ),
                    //       const SizedBox(width: 6),
  ...
  //         return _buildPropertyCard(product);
  //       },
  //     ),
  //   );
  // }

  // Replace the existing _buildTabContent method with this updated version
  Widget _buildTabContent({
    required List<dynamic> products,
    required bool isLoading,
    required String status,
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 24),
              ElevatedButton.icon(
                onPressed: isLoginRequired ? _navigateToLogin : _loadProducts,
                icon: Icon(isLoginRequired ? Icons.login : Icons.refresh),
                label: Text(isLoginRequired ? 'Login' : 'Try Again'),
                style: ElevatedButton.styleFrom(
...
// Add this method to navigate to login
  ...
  Widget _buildPropertyCard(dynamic product) {
    ...
    // Get status
    final status = _getProductStatus(productMap);
    final statusColor = _getStatusColor(status);
    final statusText = _getStatusText(status);

    // Get rejection reason if rejected
    final rejectionReason = productMap['rejectionReason'];
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // Location
                  if (productMap['address'] != null &&
                      productMap['address'].toString().isNotEmpty)
                    Row(
                      children: [
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              // View Rejection Reason (only for rejected)
              if (status == 'rejected' && product['rejectionReason'] != null)
                ListTile(
                  leading: const Icon(Icons.info_outline, color: Colors.red),
                  title: const Text('View Rejection Reason'),
                  onTap: () {
                    Navigator.pop(context);
                    _showRejectionReason(context, product);
                  },
                ),
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: Text(product['rejectionReason'] ?? 'No reason provided'),
          actions: [
            TextButton(
              onPressed: () => Navigator.pop(context),
              child: const Text('OK'),
  ...
  void _confirmStatusToggle(
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          title: Text(product['isActive'] == true
              ? 'Deactivate Listing'
              : 'Activate Listing'),
          content: Text(product['isActive'] == true
          ...
          actions: [
            ...
            TextButton(
              ...
              style: TextButton.styleFrom(
                foregroundColor:
                    product['isActive'] == true ? Colors.orange : Colors.green,
              ),
              child:
                  Text(product['isActive'] == true ? 'Deactivate' : 'Activate'),
            ),
          ],
        );
      },
