...
class _PostingDetailsState extends State<PostingDetails> with SingleTickerProviderStateMixin {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true) {
          ...
          setState(() {
            ...
            pendingProducts = products.where((p) => 
              p['status'] == 'pending' || p['status'] == null || p['isApproved'] == false
            ).toList();
            
            approvedProducts = products.where((p) => 
              p['status'] == 'approved' || p['isApproved'] == true
            ).toList();
            
            rejectedProducts = products.where((p) => 
              p['status'] == 'rejected'
            ).toList();
  ...
  void _showRejectionReason(BuildContext context, Map<String, dynamic> product) {
    showDialog(
      context: context,
      builder: (context) {
        return AlertDialog(
