...
class _NearestHouseDetailState extends State<NearestHouseDetail> {
  ...
  Map<String, dynamic>? get _userData => productData?['user'] is Map
      ? Map<String, dynamic>.from(productData!['user'])
      : null;

  Map<String, dynamic>? get _categoryData => productData?['category'] is Map
      ? Map<String, dynamic>.from(productData!['category'])
      : null;

  Map<String, dynamic>? get _attributes => productData?['attributes'] is Map
      ? Map<String, dynamic>.from(productData!['attributes'])
      : null;

  Map<String, dynamic>? get _contact => productData?['contact'] is Map
      ? Map<String, dynamic>.from(productData!['contact'])
      : null;

  List<dynamic> get _images =>
      productData?['images'] is List ? List.from(productData!['images']) : [];

  List<dynamic> get _portfolio => productData?['portfolio'] is List
      ? List.from(productData!['portfolio'])
      : [];

  List<dynamic> get _previousEvents => productData?['previousEvents'] is List
      ? List.from(productData!['previousEvents'])
      : [];

  List<dynamic> get _features => productData?['features'] is List
      ? List.from(productData!['features'])
      : [];
  ...
  String _getAgentPhone() {
    ...
    if (_contact != null) {
      if (_contact!.containsKey('callNumber') &&
          _contact!['callNumber'] != null) {
        return _contact!['callNumber'].toString().replaceAll('+', '');
      }
      if (_contact!.containsKey('whatsappNumber') &&
          _contact!['whatsappNumber'] != null) {
  ...
  String _formatAttributeLabel(String key) {
    String label =
        key.replaceAllMapped(RegExp(r'([A-Z])'), (match) => ' ${match[1]}');
  ...
  Future<void> _openWhatsApp() async {
    ...
    // 🔥 COMPANY MESSAGE
    if (categoryName == 'companies') {
      messageText =
          'Hi, I want to know more about your company: ${productData?['name'] ?? 'Company'}.\n'
          'Location: ${productData?['address'] ?? 'Unknown'}.\n'
          'Could you please share more details about your services?';
    }

    // 🔥 GOLD SHOP MESSAGE
    else if (categoryName == 'gold shops') {
      messageText =
          'Hi, I am interested in your shop: ${productData?['name'] ?? 'Shop'}.\n'
          'Location: ${productData?['address'] ?? 'Unknown'}.\n'
          'Can you share latest gold rates and available designs?';
    }
    ...
    // 🔥 DEFAULT PROPERTY MESSAGE
      ...
      messageText =
          ...
          'Location: ${productData?['address'] ?? 'Unknown location'}.\n'
  ...
  //           (key.toLowerCase().contains('price') || key == 'pricePerNight')) {
  ...
  //   text.writeln(productData?['description'] ??
  ...
  String _generateShareText() {
    ...
    // 🔥 COMPANY CUSTOM SHARE
    if (categoryName == 'companies') {
      text.writeln('🏢 *${productData?['name'] ?? 'Company'}*');
      text.writeln('');
      ...
      if (_attributes != null && _attributes!.isNotEmpty) {
        ...
        _attributes!.forEach((key, value) {
          ...
          text.writeln('• *$label:* $displayValue');
        });
      ...
      // Description
      text.writeln('📝 *About Company:*');
      text.writeln(productData?['description'] ?? '');
      text.writeln('');

      // Contact
      text.writeln('📞 *Contact Details:*');
      text.writeln('👤 ${_getAgentName()}');
      text.writeln('📱 ${_getAgentPhone()}');

      final email = _getAgentEmail();
      if (email != null) text.writeln('📧 $email');

      final website = _getAgentWebsite();
      if (website != null) text.writeln('🌐 $website');

      text.writeln('');
      text.writeln('📲 Check more in our app!');

      return text.toString();
    }

    // 🔥 NORMAL PROPERTY / HOTEL / LAND / GOLD SHOP
    text.writeln('🏠 *${productData?['name'] ?? 'Property'}*');
    ...
    // ❌ No price for companies & gold shops
    if (categoryName != 'companies' && categoryName != 'gold shops') {
      text.writeln('💰 *Price:* ${_getFormattedPrice()}');
    }
    ...
    text.writeln('');
    text.writeln('📲 Download our app for more!');
  ...
  Widget build(BuildContext context) {
    ...
    final String categoryName =
        _categoryData?['name']?.toString().toLowerCase() ?? '';
    final agentName = _getAgentName();
    final agentPhone = _getAgentPhone();
    final agentEmail = _getAgentEmail();
    final agentWebsite = _getAgentWebsite();
        ...
        children: [
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if ((isCompany || isGoldShop) &&
                          (workingHours != null || workingDays != null))
                        Container(
                      ...
                      if (_features.isNotEmpty) ...[
                        _SectionTitle(
                            isCompany ? "Key Services" : "Key Features"),
                        const SizedBox(height: 12),
                        Wrap(
                          spacing: 8,
                          runSpacing: 8,
                      ...
                      if ((isCompany || isHotel || isGoldShop) &&
                          _previousEvents.isNotEmpty) ...[
                                    ...
                                    child: Row(
                                      ...
                                      children: [
                                        const Icon(Icons.location_on,
                                            size: 14, color: Color(0xFFE33629)),
                                        const SizedBox(width: 4),
                                        Text(
                                          productData?['address']
                                                  ?.toString()
          ...
          Positioned(
            ...
            child: _BottomActionBar(
              ...
              categoryName: categoryName, // 👈 pass this
            ),
          ),
        ],
      ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    if (item['playStoreLink'] != null &&
                        item['playStoreLink'].toString().isNotEmpty)
                      _buildLinkIcon(
                    ...
                    if (item['appStoreLink'] != null &&
                        item['appStoreLink'].toString().isNotEmpty)
                      _buildLinkIcon(
                        icon: Icons.apple,
                        color: Colors.black,
                    ...
                    if (item['website'] != null &&
                        item['website'].toString().isNotEmpty)
                      _buildLinkIcon(
                        icon: Icons.language,
                        color: Colors.blue,
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          if (_attributes!.containsKey('totalRooms') &&
              _attributes!.containsKey('pricePerNight'))
          ...
          if (_attributes!.containsKey('landSize'))
            Expanded(
              child: _StatItem(
                ...
                label: 'Land Size',
                value:
                    '${_attributes!['landSize']} ${_attributes!['unit'] ?? ''}',
              ),
            ),
          if (_attributes!.containsKey('landSize') &&
              _attributes!.containsKey('landType'))
            Container(width: 1, height: 30, color: Colors.grey.shade300),
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          if (_attributes!.containsKey('bedrooms') &&
              _attributes!.containsKey('bathrooms'))
            Container(width: 1, height: 30, color: Colors.grey.shade300),
...
class _BottomActionBar extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final isCompany = categoryName.toLowerCase() == 'companies';
