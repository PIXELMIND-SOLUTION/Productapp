...
//     // Start progress timer based on media type
//     if (story.mediaType == 'video') {
//       // Video duration - will be set when ExoVideoPlayer initializes
//       // For now, use default duration, we'll update when we get the duration
//       _startProgressTimer(duration: 15.0); // Default video duration
//     } else {
//       if (mounted) {
//         setState(() => _isLoading = false);
//         _startProgressTimer(); // 10 seconds for images
//       }
//     }
//   }
...
//   void _onVideoDuration(double duration) {
//     // Restart timer with video duration when it's available
//     _progressTimer?.cancel();
//     _startProgressTimer(duration: duration);
//   }
...
class _StoryViewerScreenState extends State<StoryViewerScreen> {
  ...
  Future<void> _loadCurrentStory() async {
    ...
    // Start progress timer based on media type
    if (story.mediaType == 'video') {
      // For videos, we'll use a default duration
      // The timer will be managed by the video player's state
      if (mounted) {
        setState(() => _isLoading = false);
        _startProgressTimer(duration: 15.0); // 15 seconds for videos
      }
    } else {
      if (mounted) {
        setState(() => _isLoading = false);
        _startProgressTimer(duration: 10.0); // 10 seconds for images
      }
    }
  }
